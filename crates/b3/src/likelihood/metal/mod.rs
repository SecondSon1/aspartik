...
/// CPU-side types (f64, matches the Calculator trait)
type Row = [f64; 4];
type Transition = [f64; 16];

/// GPU-side types (f32, Metal doesn't support f64 buffers on most Apple GPUs)
type GpuRow = [f32; 4];
...
// SAFETY: access is serialised by the Mutex in GenericLikelihood.
unsafe impl Send for MetalLikelihood {}
unsafe impl Sync for MetalLikelihood {}
...
impl Calculator<4, f64> for MetalLikelihood {
    fn likelihood(
        ...
        let children_u32: Vec<u32> = children
            ...
            .collect();
        // Convert f64 transition matrices to f32 for the GPU.
        let tms_f32: Vec<GpuTransition> = {
            let tms_flat: &[Transition] = bytemuck::cast_slice(&tms);
            tms_flat
                .iter()
                .map(|t| t.map(|v| v as f32))
                .collect()
        };

        // Upload host data to shared-mode buffers (no DMA needed on Apple
        // Silicon; regular writes suffice).
        ...
        let likelihoods_f32: Vec<f32> = unsafe {
            read_slice(&self.likelihoods, self.num_patterns as usize)
        };
        ...
        let total: f64 = likelihoods_f32
            ...
            .map(|((l, &scale), &weight)| {
                (f64::from(*l) - f64::from(scale)) * f64::from(weight)
            })
            .sum();
...
impl MetalLikelihood {
    ...
    fn encode_update_leaves(&self, leaves_end: u32) -> Result<()> {
        ...
        unsafe {
            ...
            enc.setBuffer_offset_atIndex(Some(&self.projections), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&self.nodes), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&self.transitions), 0, 3);
        }
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize { width: num_pattern_blocks, height: leaves_end as usize, depth: 1 },
            MTLSize { width: block_size, height: 4, depth: 1 },
        );
        enc.endEncoding();
        cmd.commit();
        Ok(())
    ...
    fn encode_update_all(
        ...
        unsafe {
            ...
            enc.setBuffer_offset_atIndex(Some(&self.scale_sums), 0, 3);
            self.set_u32(&enc, self.num_updated_nodes, 4);
            enc.setBuffer_offset_atIndex(Some(&self.nodes), 0, 5);
            enc.setBuffer_offset_atIndex(Some(&self.children), 0, 6);
            enc.setBuffer_offset_atIndex(Some(&self.transitions), 0, 7);
            self.set_u32(&enc, leaves_end, 8);
            self.set_u32(&enc, internals_start, 9);
        }
        enc.dispatchThreadgroups_threadsPerThreadgroup(
            MTLSize { width: num_pattern_blocks as usize, height: 1, depth: 1 },
            MTLSize { width: block_size as usize, height: 1, depth: 1 },
        );
        enc.endEncoding();
        cmd.commit();
        Ok(())
    ...
    fn encode_update_likelihoods(
        ...
        unsafe {
            enc.setBuffer_offset_atIndex(Some(&self.projections), 0, 0);
            enc.setBuffer_offset_atIndex(Some(&self.likelihoods), 0, 1);
            enc.setBuffer_offset_atIndex(Some(&self.scales), 0, 2);
            enc.setBuffer_offset_atIndex(Some(&self.scale_sums), 0, 3);
            ...
            enc.setBytes_length_atIndex(
                NonNull::new(frequencies.as_ptr() as *mut c_void).unwrap(),
        ...
        unsafe {
            if accept {
                enc.setBuffer_offset_atIndex(
                    Some(&self.projections), 0, 0,
                );
                enc.setBuffer_offset_atIndex(
                    Some(&self.projections_backup), 0, 1,
                );
                enc.setBuffer_offset_atIndex(Some(&self.scales), 0, 2);
                enc.setBuffer_offset_atIndex(
                    Some(&self.scales_backup), 0, 3,
                );
            } else {
                enc.setBuffer_offset_atIndex(
                    Some(&self.projections_backup), 0, 0,
                );
                enc.setBuffer_offset_atIndex(
                    Some(&self.projections), 0, 1,
                );
                enc.setBuffer_offset_atIndex(
                    Some(&self.scales_backup), 0, 2,
                );
                enc.setBuffer_offset_atIndex(Some(&self.scales), 0, 3);
            }
    ...
    fn set_u32(
        ...
        unsafe {
            enc.setBytes_length_atIndex(
                NonNull::new(std::ptr::addr_of!(value) as *mut c_void)
                    .unwrap(),
                size_of::<u32>(),
                index,
            );
    ...
    pub fn new(
        ...
        let full_src = format!(
            "#define NUM_PATTERNS {num_patterns}u\n\
        ...
        let library = device
            ...
            .map_err(|e| {
                anyhow!("Metal shader compilation failed: {e:?}")
            })?;
        ...
        let update_likelihoods_pipeline =
            compile_pipeline(&device, &library, "update_likelihoods")?;
        let copy_projections_pipeline =
            compile_pipeline(&device, &library, "copy_projections")?;
        ...
        let children =
            alloc_buf::<u32>(&device, num_internals * 2)?;
        let transitions =
